...
function Ornn:Combo()
    ...
    if target and Engine:SpellReady("HK_SPELL1") and self.ComboR.Value == 1 then
        local enemiesAround = self:EnemiesInRange(target.Position, 400)
        if enemiesAround >= self.ComboRSlider.Value then
            local CastPos, Target = Prediction:GetCastPos(myHero.Position, self.RRange, self.RSpeed, self.RWidth, self.RDelay, 0, 0, 0.3, 1)
            if CastPos then
