...
function Kindred:__init()
    ...
    self.SimpleLaneClearUseQ    = self.LaneClearMenu:AddCheckbox("Use simple Q in Laneclear", 1)
    ...
    self.SimpleLaneClearUseW    = self.LaneClearMenu:AddCheckbox("Use simple W in Laneclear", 1)
    ...
    self.SimpleLaneClearUseE    = self.LaneClearMenu:AddCheckbox("Use simple E in Laneclear", 1)
...
function Kindred:SaveSettings()
    ...
    SettingsManager:AddSettingsInt("Use simple Q in Laneclear", self.SimpleLaneClearUseQ.Value)
    ...
    SettingsManager:AddSettingsInt("Use simple W in Laneclear", self.SimpleLaneClearUseW.Value)
    ...
    SettingsManager:AddSettingsInt("Use simple E in Laneclear", self.SimpleLaneClearUseE.Value)
...
function Kindred:LoadSettings()
    ...
    self.SimpleLaneClearUseQ.Value  = SettingsManager:GetSettingsInt("Laneclear", "Use simple Q in Laneclear")
    self.LaneClearQMana.Value       = SettingsManager:GetSettingsInt("Laneclear", "Minimum % mana to use Q")
    self.SimpleLaneClearUseW.Value  = SettingsManager:GetSettingsInt("Laneclear", "Use simple W in Laneclear")
    ...
    self.SimpleLaneClearUseE.Value  = SettingsManager:GetSettingsInt("Laneclear", "Use simple E in Laneclear")
...
function Kindred:Combo()
	...
	if self.UseQCombo.Value == 1 and Engine:SpellReady("HK_SPELL1") then
        ...
        if Target then
            ...
            if self:EnemyVSAlly(self:QCastPos(), self:GetAttackRange()) > 1 then
                return
            end
            ...
            if self:GetDistance(Target.Position, myHero.Position) > (self:GetAttackRange()) then
...
function Kindred:Laneclear()
    ...
    if Engine:SpellReady('HK_SPELL1') and self.SimpleLaneClearUseQ.Value == 1 then
        local target = Orbwalker:GetTarget("Laneclear", self:GetAttackRange() + 240)
        if target then
            if self:GetDistance(myHero.Position, target.Position) <= self:GetAttackRange() + 240 then
    ...
    if Engine:SpellReady('HK_SPELL2') and self.SimpleLaneClearUseW.Value == 1 then
    ...
    if self.SimpleLaneClearUseE.Value == 1 and Engine:SpellReady('HK_SPELL3') then
        ...
        if self.LaneClearUseE.Value == 1 and Engine:SpellReady('HK_SPELL3') then
            local eRange = self:GetAttackRange() + 30
            local target = Orbwalker:GetTarget("Laneclear", eRange)
            ...
            if target then
                ...
                if myHero.Mana >= ELaneCondition then
                    for I, Name in pairs(CampNames) do
                        if Name == target.Name then
                            if target.Health > (DamageLib.Kindred:AADmg(target, self:HasInfinityEdge()) * 3) then
                                Engine:CastSpell("HK_SPELL3", target.Position ,1)
                                return
                            end
                        end
...
function Kindred:DrawHitMark(target)
    ...
    if PassiveTimerBuff > 0 then
        ...
        local PassiveTime = 8 - GameClock.Time + self.TimerStart5
    ...
    for I, Minion in pairs(Minions) do
        if Minion.Team ~= myHero.Team then
            if self:GetDistance(Minion.Position, myHero.Position) < 700 then
                if Minion.BuffData:GetBuff("kindredhittracker").Count_Alt > 0 then
