...
function Soraka:W()
	if Engine:SpellReady("HK_SPELL2") then
		local HealTarget = SupportLib:GetShieldTargetWithTable(self.WRange, self.WTargets)
		if HealTarget then
...
function Soraka:R()
	if Engine:SpellReady("HK_SPELL4") then
		local HealTarget = SupportLib:GetShieldTargetWithTable(self.RRange, self.RTargets)
		if HealTarget then
...
function Soraka:OnTick()
	...
	for _, Ally in pairs(Allies) do
		if string.len(Ally.ChampionName) > 1 and self.WTargets[Ally.Index] == nil and Ally.Index ~= myHero.Index then
			self.WTargets[Ally.Index] 		= self.WSettings:AddSlider(Ally.ChampionName , 80, 0, 100, 1)
		end
