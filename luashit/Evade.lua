...
function Evade:IsInAOESkillShot(EndPos, Spell)
	...
	if self:GetDist(playerPos, EndPos) <= Spell.Radius + RadiusAddition then
		return true
	else
		return false
	end
...
function Evade:IsDodgePosInSpell(evadePos, EndPos, Spell)
	local RadiusAddition = myHero.CharData.BoundingRadius * 1.55
	local mousePos = GameHud.MousePos
	local waypoint = myHero.AIData.TargetPos
	if self:GetDist(evadePos, EndPos) <= Spell.Radius + RadiusAddition then
		return true
	else
		return false
	end
...
function Evade:IsInSkillShot(StartPos, EndPos, PosToCheck, Spell, myHeroTeamObject)
	...
	if Spell.Type == 0 then
		if myHeroTeamObject == nil then
			local RadiusAddition = myHero.CharData.BoundingRadius * 2.2
			local PlayerPos = myHero.Position
			local MousePos = GameHud.MousePos
...
function Evade:GetDodgePos(searchsize)
	...
	for i, Spell in pairs(self.SpellsToDodge) do
		local isInSkillShot = self:IsInSkillShot(Spell["Data"]["StartPos"], Spell["Data"]["EndPos"], GameHud.MousePos, Spell["Data"]["Spell"], nil)
		if not isInSkillShot then
			...
			if not self:IsInSkillShot(Spell["Data"]["StartPos"], Spell["Data"]["EndPos"], Point, Spell["Data"]["Spell"], nil) and self:GetDist(myHero.Position, GameHud.MousePos) > Spell["Data"]["Spell"].Radius then
        ...
        for x = 0, 15 do
            if x == 0 then
				...
				for i, STD in pairs(self.SpellsToDodge) do
					-- if not self:IsInSkillShot(STD["Data"]["StartPos"], STD["Data"]["EndPos"], evadePos, STD["Data"]["Spell"], nil) and self:IsValidDodgePos(evadePos) then
					...
					if dodgePosFound then
						...
						if not self:IsInSkillShot(STD["Data"]["StartPos"], STD["Data"]["EndPos"], dodgePos, STD["Data"]["Spell"], nil) and self:IsValidDodgePos(dodgePos) then
							return dodgePos, STD["Data"]["Spell"]
						else
							dodgePosFound = false
						end
					end
					if not dodgePosFound and not self:IsInSkillShot(STD["Data"]["StartPos"], STD["Data"]["EndPos"], evadePos, STD["Data"]["Spell"], nil) and self:IsValidDodgePos(evadePos) then
						dodgePos = evadePos
						dodgePosFound = true
	...
	for I, Hero in pairs(Heros) do
		if Hero.Team ~= myHero.Team then
			...
			if Spell ~= nil and self.DodgeSpells[SpellName] and self.DodgeSpells[SpellName].Value == 1 then
				...
				local timeToHit = (distanceSpellToPlayer / Spell.Speed + Spell.Delay) -- might want to add Spell.Delay ? not sure
				timeToHit = timeToHit + (Spell.Delay / 2)
	...
	if GameHud.Minimized == false and GameHud.ChatOpen == false then
		...
		if Point and Spell then
			...
			if not self:CanEvade(Spell.Type, Point) then
				local Point2, Spell2 = self:GetDodgePos(300)
